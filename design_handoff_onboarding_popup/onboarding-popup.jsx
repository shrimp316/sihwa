// onboarding-popup.jsx — 최초 접속 시 조작 안내 팝업.
// localStorage 'sihwa_guide_seen' 키로 1회만 표시.

const { useState: useOB, useEffect: useOBE } = React;

// variant: 'portrait' (기본, 모바일) | 'landscape' (PC 가로 스프레드)
function OnboardingPopup({ onDismiss, variant = 'portrait' }) {
  const [visible, setVisible] = useOB(false);
  const [exiting, setExiting] = useOB(false);
...
// ── SVG 일러스트 ───────────────────────────────────────────────────────────
function BookIllustration({ variant = 'portrait' }) {
  return variant === 'landscape'
    ? <BookIllustrationLandscape />
    : <BookIllustrationPortrait />;
}
...
function BookIllustrationLandscape() {
  ...
  // 스크롤 가능 영역 — 페이지 중앙 박스
  const zoneInset = 14;        // 페이지 내 여백
  const zoneX  = (px) => px + zoneInset;
  const zoneY  = pY + 22;
...
Object.assign(window, { OnboardingPopup });
