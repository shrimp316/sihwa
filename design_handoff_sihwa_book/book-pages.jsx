// PageContent — renders ONE page of the bound book based on page descriptor.
// Pages are page-size-agnostic; they use rem/em + percentages so the same
// markup renders crisply in both portrait (single page) and landscape
// (two-up spread, narrower per page).

function PageContent({ page, side /* 'left' | 'right' | 'single' */, totalPages }) {
  if (!page) return null;
  switch (page.type) {
    case 'front-cover':       return <FrontCover />;
    case 'half-title':        return <HalfTitle />;
    case 'title-page':        return <TitlePage />;
    case 'copyright':         return <Copyright />;
    case 'toc-title':         return <TocTitle />;
    case 'toc-quarter':       return <TocQuarter quarter={page.quarter} qi={page.qi} />;
    case 'quarter-divider':   return <QuarterDivider quarter={page.quarter} qi={page.qi} />;
    case 'quarter-intro':     return <QuarterIntro quarter={page.quarter} />;
    case 'round-divider':     return <RoundDivider round={page.round} />;
    case 'poem':              return <PoemPage page={page} />;
    case 'free-divider':      return <FreeDivider quarter={page.quarter} />;
    case 'free-poem':         return <PoemPage page={page} />;
    case 'end':               return <EndPage />;
    case 'colophon':          return <Colophon totalPages={totalPages} />;
    case 'back-cover':        return <BackCover />;
    case 'blank':             return <BlankPage kind={page.kind} />;
    default:                  return null;
  }
}
...
function FrontCover() {
  ...
  return (
    <div className="pg pg-cover">
      {ctx.front
        ? <img src={ctx.front} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }} alt="앞표지" />
        : <image-slot id="front-cover-photo" placeholder="시화 앞표지 사진 — 드래그하거나 클릭" style={{ width:'100%', height:'100%', display:'block' }} />
      }
    </div>
  );
}
function BackCover() {
  ...
  return (
    <div className="pg pg-cover">
      {ctx.back
        ? <img src={ctx.back} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }} alt="뒷표지" />
        : <image-slot id="back-cover-photo" placeholder="시화 뒷표지 사진 — 드래그하거나 클릭" style={{ width:'100%', height:'100%', display:'block' }} />
      }
    </div>
  );
}
...
function TitlePage() {
  return (
    <div className="pg pg-center">
      ...
      <div className="tp-title">시화<span className="tp-han">詩和</span></div>
...
function TocQuarter({ quarter, qi }) {
  const rounds = SIHWA_DATA.rounds
    .filter(r => r.quarterId === quarter.id)
    .sort((a, b) => a.order - b.order);
  const free = SIHWA_DATA.freePoems
  ...
  return (
    <div className="pg pg-toc">
      ...
      <div className="toc-q-list">
        {rounds.map(r => {
          const rPoems = SIHWA_DATA.poems
            .filter(p => p.roundId === r.id)
            .sort((a, b) => a.order - b.order);
          if (!rPoems.length) return null;
          return (
            <div key={r.id} className="toc-q-round">
              <div className="toc-q-round-label">{r.num}회차 — {r.title}</div>
              {rPoems.map(p => (
                <div key={p.id} className="toc-q-item">
        ...
        {free.length > 0 && (
          <div className="toc-q-round">
            ...
            {free.map(f => (
              <div key={f.id} className="toc-q-item">
...
function QuarterDivider({ quarter, qi }) {
  return (
...
// ─── Poem entry ────────────────────────────────────────────────────────
// page.chunkIdx   : 0 = first chunk (shows title/poet header)
//                   1+ = continuation (shows abbreviated header)
// page.totalChunks: 1 = single-page poem (no 'continued' indicator)
function PoemPage({ page }) {
  const poem = page.poem || page.freePoem;
  const body = page.bodyChunk ?? poem.body;
  const isFirst = (page.chunkIdx ?? 0) === 0;
  const isContinued = !isFirst;
  const hasMore = page.totalChunks > 1 && page.chunkIdx < page.totalChunks - 1;
  ...
  return (
    <div className="pg pg-poem">
      ...
      {hasMore && (
        <div className="po-continues">다음 페이지로 이어집니다 →</div>
      )}
    </div>
  );
}
